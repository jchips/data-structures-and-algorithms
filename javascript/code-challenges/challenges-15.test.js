...
const toTitleCase = (arr) => {
  return arr.map(string => {
    let capitalizeLetter = string.split('');
    capitalizeLetter.splice(0, 1, capitalizeLetter[0].toUpperCase());
...
let biggerThanLuke = (arr) => {
  ...
  let newArray = arr.filter(char => parseInt(char.mass) > lukeMass)
    .map(char => char.name);
