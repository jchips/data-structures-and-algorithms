...
const count = (target, input) => {
  ...
  input.map(arr => {
    ...
    targetCount += filteredArr.length;
  });
  return targetCount;
};
...
let findMaleAndFemale = (data) => {
  let string = '';
  let firstChar = data.find(char => char.gender === 'male' || char.gender === 'female');
  string += firstChar.name;
  data.map(char => {
    if((char.gender === 'male' || char.gender === 'female') && char !== firstChar) {
      string += ' and ' + char.name;
    }
  });
  return string;
};
...
let findShortest = (data) => {
  let startPoint = parseInt(data[0].height);
  let shortestHeight = data.reduce((acc, char) => {
    if (acc < parseInt(char.height)) {
      return acc;
    }
    return char.height;
  }, startPoint);
  let char = data.filter(char => char.height === shortestHeight + '');
  return char[0].name;
};
