...
const longestString = (arr) => {
  let storeLongestString = arr[0];
  arr.forEach(string => {
    if (string.length > storeLongestString.length) {
      storeLongestString = string;
    }
  });
  let longestStringIdx = arr.indexOf(storeLongestString);
...
const standardizePhoneNumbers = (arr) => {
  return arr.map(phoneNum => {
    let pattern = /\d/gm;
    let arr = phoneNum.match(pattern);
    return arr.join('');
  });
};
