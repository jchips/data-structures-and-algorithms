...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.map(ingredient => {
    let deleteFirstWord = ingredient.slice(ingredient.indexOf(' ')).slice(1);
    result.push(deleteFirstWord.slice(deleteFirstWord.indexOf(' ')).slice(1));
  });
  return result;
};
...
const splitFoods = (recipe) => {
  ...
  recipe.ingredients.map(ingredient => {
    ...
    let editedIngredient = arr.join(' ');
    result.push(editedIngredient);
  });
  return result;
};
...
const stepActions = (recipe) => {
  ...
  recipe.steps.map(step => {
    let editedStep = step.split(' ');
    let length = editedStep.length;
    for (let i = 1; i < length; i++) {
      editedStep.pop();
    }
    editedStep = editedStep.join(' ');
    result.push(editedStep);
  });
  return result;
};
...
const removeEvenValues = (arr) => {
  arr.map(num => {
    if (num % 2 === 0) {
      return arr.splice(arr.indexOf(num), 1);
    }
  });
  return arr;
};
